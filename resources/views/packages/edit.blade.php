    ...
    <div>
        <div class="h-full rounded-md">
            <div class="rounded-xl bg-white p-5">
                ...
                <form action="{{ route('packages.update', $packages->id) }}" method="post" autocomplete="off"
                    enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                    <input type="hidden" name="oldPicture" value="{{ $packages->picture }}">
                    <div class="grid lg:grid-cols-2 gap-6">
                        <div>
                            <div class="mb-5">
                                <label for="name" class="text-abu mb-3 block">Name</label>
                                <input type="text" id="name" name="name"
                                    value="{{ old('name') ? old('name') : $packages->name }}"
                            ...
                            <div class="mb-5">
                                ...
                                <textarea id="description" rows="6" name="description"
                                    class="border-blues focus:border-blues focus:ring-blues block w-full rounded-lg border p-2.5 text-sm text-gray-900 outline-blue-400">{{ old('description') ? old('description') : $packages->description }}</textarea>
                                @error('description')
                                    <div class="text-reds mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-x-5">
                                <div class="mb-5">
                                    ...
                                    <input type="text" id="size" name="size"
                                        value="{{ old('size') ? old('size') : $packages->size }}"
                                        class="border-blues focus:border-blues focus:ring-blues block w-full rounded-lg border p-2.5 text-sm text-gray-900 outline-blue-400"
                                        placeholder="" />
                                    @error('size')
                                        <div class="text-reds mt-1">{{ $message }}</div>
                                ...
                                <div class="mb-5">
                                    ...
                                    <input type="text" id="color" name="color"
                                        value="{{ old('color') ? old('color') : $packages->color }}"
                                        class="border-blues focus:border-blues focus:ring-blues block w-full rounded-lg border p-2.5 text-sm text-gray-900 outline-blue-400"
                                        placeholder="" />
                                    @error('color')
                                        <div class="text-reds mt-1">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            <div class="mb-3">
                                ...
                                <input type="text" id="account" name="account"
                                    value="{{ old('account') ? old('account') : $packages->account }}"
                                    class="border-blues focus:border-blues focus:ring-blues block w-full rounded-lg border p-2.5 text-sm text-gray-900 outline-blue-400"
                                    placeholder="" />
                                @error('account')
                                    <div class="text-reds mt-1">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" id="box_number" name="box_number"
                                    value="{{ old('box_number') ? old('box_number') : $packages->box_number }}"
                                    class="border-blues focus:border-blues focus:ring-blues block w-full rounded-lg border p-2.5 text-sm text-gray-900 outline-blue-400"
                                    placeholder="" />
                                @error('box_number')
                                    <div class="text-reds mt-1">{{ $message }}</div>
